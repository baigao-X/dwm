...
refresh() {
    ...
    _icons=''; _cpu=''; _mem=''; _date='';# 重置所有模块的状态为空
    source $tempfile                                                     # 从 temp 文件中读取模块的状态
    xsetroot -name "$_icons$_cpu$_mem$_date"      # 更新状态栏
}
...
cron() {
    ...
    while true; do
        ...
        [ $((i % 20)) -eq 0 ]  && to=(${to[@]} cpu mem icons)           # 每 20秒  更新 cpu mem vol icons
        ...
        [ $i -lt 30 ] && to=(cpu mem date icons)            # 前 30秒  更新所有模块
...
case $1 in
    ...
    updateall|check) update icons cpu mem date ;;
